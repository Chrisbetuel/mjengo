...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    error_log("Group registration started by user ID: " . $_SESSION['user_id']);
    ...
    if (!empty($group_members)) {
        ...
        foreach ($group_members as $member_id) {
            if (!empty($member_id)) {
                $member_id = sanitize($member_id);
                // Check if user exists and is not the leader
        ...
        $group_members = array_unique($valid_members);

        // Check if any selected members are already in another group for this challenge
        if ($group_type === 'existing' && !empty($challenge_id)) {
            foreach ($group_members as $member_id) {
                $stmt = $pdo->prepare("SELECT gm.id FROM group_members gm JOIN groups g ON gm.group_id = g.id WHERE gm.user_id = ? AND g.challenge_id = ?");
                $stmt->execute([$member_id, $challenge_id]);
                if ($stmt->fetch()) {
                    $errors[] = 'One or more selected users are already in a group for this challenge.';
                    error_log("Group registration validation failed: User ID $member_id already in group for challenge ID $challenge_id");
                    break;
                }
            }
        }
    ...
    if (empty($group_name)) {
        ...
        if ($group_type === 'existing') {
            ...
            if (empty($challenge_id)) {
                ...
                if (!$challenge) {
                    ...
                    error_log("Group registration validation failed: Challenge ID $challenge_id not available");
                } else {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        error_log("Group registration validation failed: User ID {$_SESSION['user_id']} already has group for challenge ID $challenge_id");
                    }
                }
            }
        ...
        if (empty($errors)) {
            ...
            try {
                ...
                if (!empty($group_members)) {
                    foreach ($group_members as $member_id) {
                        $stmt = $pdo->prepare("INSERT INTO group_members (group_id, user_id, status) VALUES (?, ?, 'invited')");
                        ...
                        $action_url = "accept_invitation.php?id=" . $pdo->lastInsertId();
                        $action_text = "View Invitation";

                        sendNotification($member_id, $notification_title, $notification_message, 'group_invitations', $action_url, $action_text);
                    }
                }
...
// Fetch all users except current user for member selection
            ...
            <div class="register-card">
                ...
                <div class="register-body">
                    ...
                    <form method="POST" id="groupRegistrationForm" novalidate>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Select Group Members (Optional)</label>
                            <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                                <?php if (empty($all_users)): ?>
                                    <p class="text-muted mb-0">No other users available to add to the group.</p>
                                <?php else: ?>
                                    <?php foreach ($all_users as $user): ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="group_members[]" value="<?php echo $user['id']; ?>" id="user_<?php echo $user['id']; ?>">
                            ...
                            <small class="text-muted">Selected members will be invited to join your group. You can add more members later.</small>
                        </div>
